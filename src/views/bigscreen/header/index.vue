<template>
  <div class="relative">
    ...
    <div class="absolute left-[46.88vw] top-1/2 -translate-y-1/3">
      <buttonBorder class="h-[4.35vh] w-[7.45vw]" :url="btnImg('screen1')" @click="changeScreen('screen1')"
        ><span
          class="btn-text absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-nowrap"
          :style="screenStateStore.screenState.screen1 ? 'font-weight: 700' : 'font-weight: normal'"
          >XXXX</span
        >
      </buttonBorder>
    </div>
    <div class="btn-text absolute left-[54.69vw] top-1/2 -translate-y-1/3">
      <buttonBorder class="h-[4.35vh] w-[7.45vw]" :url="btnImg('screen2')" @click="changeScreen('screen2')"
        ><span
          class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-nowrap"
          :style="screenStateStore.screenState.screen2 ? 'font-weight: 700' : 'font-weight: normal'"
          >XXXX</span
        >
      </buttonBorder>
    </div>
    <div class="btn-text absolute left-[62.69vw] top-1/2 -translate-y-1/3">
      <buttonBorder class="h-[4.35vh] w-[7.45vw]" :url="btnImg('screen3')" @click="changeScreen('screen3')"
        ><span
          class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-nowrap"
          :style="screenStateStore.screenState.screen3 ? 'font-weight: 700' : 'font-weight: normal'"
          >XXXX</span
        >
      </buttonBorder>
    </div>
...
function changeScreen(type) {
  screenStateStore.changeScreenState(type);
}
function btnImg(type) {
  if (screenStateStore.screenState[type]) {
    return new URL('../../../assets/images/header-button/header-active.svg', import.meta.url).href;
  }
  return new URL('../../../assets/images/header-button/header.svg', import.meta.url).href;
}
